...
from utils import AliasedGroup, abort_if_false, handle_job, CLIException, str_fsize
...
def list(ctx):
    ...
    try:
        ...
        for x in data:
            ...
            click.secho(u" ↳ Type: {} {}".format(x["type"].capitalize(), x["fstype"]), fg="white")
            click.secho(u" ↳ Size: {}".format(str_fsize(x["size"])), fg="white")
            click.secho(u" ↳ Encrypted: {}".format("Yes" if x["crypt"] else "No"), fg="white")
            click.secho(u" ↳ Mounted: {}".format("At " + x["mountpoint"] if x["mountpoint"] else "No"), fg="white")

@filesystems.command()
@click.argument("name")
@click.option("--size", required=True, type=int, prompt="Size of the new disk (in MB)", help="Size of the new disk (in MB)")
@click.option("--encrypt", is_flag=True, prompt="Encrypt this filesystem?", help="Encrypt this filesystem?")
