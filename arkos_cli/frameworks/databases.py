...
def list(ctx):
    ...
    try:
        ...
        for x in sorted(dbs, key=lambda x: x["id"]):
            smsg = click.style(" (" + x["type_name"] + ")", fg="yellow")
            click.echo(click.style(x["id"], fg="green") + smsg)
...
def list_types(ctx):
    ...
    try:
        ...
        for x in sorted(dbs, key=lambda x: x["id"]):
            smsg = click.style(" (Running)" if x["state"] else " (Stopped)",
                               fg="yellow")
            click.echo(click.style(x["name"], fg="green") + smsg)
...
def chmod(ctx, user_name, db_name, grant):
    ...
    try:
        if ctx.obj["conn_method"] == "remote":
            cmd = ctx.obj["client"].databases.user_chmod
